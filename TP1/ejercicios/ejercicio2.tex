...
\end{figure}
\\
\par{Ahora veamos cómo sería este proceso con $T = 2$.
...
\begin{algorithmic}
  \Function{IndiceDeMayorXQueMata}{t: int, indiceDeObjetivo: int, e: $Arreglo<Tupla<int,int>>$} $\to j$ : \texttt{int}
  	\State $int~ distanciaDeDaño \gets e[indiceDeObjetivo].x + t$ \Comment $\mathcal{O}(1)$
	\State $int~ i \gets indiceDeObjetivo - 1$ \Comment $\mathcal{O}(1)$
	\While{$(i \geq 0) \wedge (((e[i]).x) \leq distanciaDeDaño$} \Comment $\mathcal{O}(indiceDeObjetivo)$
...
\begin{algorithmic}
  \Function{Genkidama}{t: int, n: int, e: $Arreglo<Tupla<int,int>>$}
	...
	\While{$hayAlgunoVivo$} \Comment $\mathcal{O}(indiceDeObjetivo)$
		...
		\State $el +1 de arriba surge de que en el enunciado se enumeran desde el 1 y nosotros enumeramos desde 0$
		\State $hayAlgunoVivo \gets \neg(indiceDeMayorXQueMata(t, indiceDeObjetivo, e) == 0)$ \Comment $\mathcal{O}(1)$
		\State $indiceDeObjetivoPorArea \gets indiceDeMayorXQueMata(t, indiceDeObjetivo, e) - 1$ \Comment $\mathcal{O}(1)$
		\State genkidamasUtilizadas++ \Comment $\mathcal{O}(1)$
	\EndWhile
	\State $std::cout << genkidamasUtilizadas << std::endl$
	\State $int~ h \gets 0$
	\While{$h < genkidamasUtilizadas$}
			\State $std::cout << atacados[h]$
			\State $h++$
			\If{$h < genkidamasUtilizadas$}
					\State $std::cout << " "$
			\EndIf
	\EndWhile
	\State $std::cout << std::endl$
\EndFunction
\end{algorithmic}
\underline{Complejidad:} $\mathcal{O}(indiceDeObjetivo)$\\
